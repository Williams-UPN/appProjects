...
CREATE
OR REPLACE FUNCTION "public"."_crear_cronograma_aux"("p_cliente_id" bigint) RETURNS "void" LANGUAGE "plpgsql" AS $ $ DECLARE v_plazo INTEGER;
...
SELECT
  ...
  cuota_diaria,
  ultima_cuota,
  fecha_primer_pago :: date INTO v_plazo,
  v_std,
  v_last,
  v_fecha_inicio
FROM
...
CREATE
OR REPLACE FUNCTION "public"."_generar_cronograma_trg"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER
SET
  "search_path" TO 'public' AS $ $ declare v_hoy date := date(timezone('America/Lima', now()));

v_estado text;

begin PERFORM public.crear_cronograma_para_cliente(new.id);
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_actualizar_estado"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE cid bigint := COALESCE(NEW.cliente_id, OLD.cliente_id);
...
UPDATE
  public.cronograma
SET
  fecha_pagado = NEW.fecha_pago :: date
WHERE
  cliente_id = cid
...
SELECT
  COALESCE(SUM(monto_pagado), 0) INTO total_pagado
FROM
  public.pagos
WHERE
  cliente_id = cid;
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_cierre_historial"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_plazo INTEGER;

BEGIN
SELECT
  plazo_dias INTO v_plazo
FROM
  public.clientes
WHERE
  id = NEW.cliente_id;
...
IF TG_OP = 'INSERT'
AND NEW.numero_cuota = v_plazo THEN PERFORM public.crear_historial_cerrado(NEW.cliente_id);
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_cierre_por_refi"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ BEGIN PERFORM public.crear_historial_cerrado(OLD.id);
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_generar_cronograma"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_hoy date := (now() AT TIME ZONE 'America/Lima') :: date;

v_fp date := NEW.fecha_primer_pago;

-- ya es DATE
v_estado text;

BEGIN PERFORM public._crear_cronograma_aux(NEW.id);

UPDATE
  public.clientes
SET
  saldo_pendiente = NEW.total_pagar,
  ultima_cuota_numero = 0
WHERE
  id = NEW.id;

IF v_fp > v_hoy THEN v_estado := 'proximo';
...
ELSIF EXISTS (
  ...
  WHERE
    ...
    AND fecha_venc = v_hoy
    AND fecha_pagado IS NULL
) THEN v_estado := 'pendiente';

ELSE v_estado := 'al_dia';
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_log_pago"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_fecha_venc DATE;

v_atraso INTEGER;

BEGIN
SELECT
  fecha_venc INTO v_fecha_venc
FROM
  public.cronograma
WHERE
...
INSERT INTO
  public.historial_eventos(cliente_id, tipo_evento, descripcion)
VALUES
  (
    NEW.cliente_id,
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_refinanciar_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_total_pagado NUMERIC;
...
BEGIN -- Métricas del crédito antiguo
SELECT
  COALESCE(SUM(p.monto_pagado), 0),
  COALESCE(
    MAX(
      GREATEST(
        (p.fecha_pago AT TIME ZONE 'America/Lima') :: date - cr.fecha_venc,
        0
      )
    ),
    0
  ),
  COUNT(*) INTO v_total_pagado,
  v_max_atraso,
  v_count_incid
FROM
  public.pagos p
...
SELECT
  MIN(fecha_venc) INTO v_fecha_inicio
FROM
  public.cronograma
WHERE
  cliente_id = OLD.id;
...
VALUES
  (
    ...
    GREATEST(
      0,
      LEAST(100, 100 - v_max_atraso * 2 - v_count_incid * 5)
    )
  );
...
VALUES
  (
    ...
    format(
      ...
      NEW.plazo_dias,
      (
        NEW.fecha_primer_pago AT TIME ZONE 'America/Lima'
      ) :: date
    )
  );
...
CREATE
OR REPLACE FUNCTION "public"."_trigger_regenerar_cronograma"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ BEGIN
DELETE FROM
  public.cronograma
WHERE
  cliente_id = NEW.id;
...
CREATE
OR REPLACE FUNCTION "public"."_validar_recalcular_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_tasa NUMERIC;
...
BEGIN IF NEW.monto_solicitado <= 0 THEN RAISE EXCEPTION 'monto_solicitado debe ser > 0';

ELSIF NEW.plazo_dias NOT IN (12, 24) THEN RAISE EXCEPTION 'plazo_dias inválido';
...
v_tasa := CASE
  WHEN NEW.plazo_dias = 12 THEN 10
  ELSE 20
END;
...
CREATE
OR REPLACE FUNCTION "public"."_validar_y_recalcular_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE v_today date := (now() AT TIME ZONE 'America/Lima') :: date;
...
BEGIN IF NEW.fecha_primer_pago < v_today THEN RAISE EXCEPTION 'fecha_primer_pago (%) debe ser ≥ hoy (Lima: %)',
NEW.fecha_primer_pago,
v_today;

END IF;

IF NEW.monto_solicitado <= 0 THEN RAISE EXCEPTION 'monto_solicitado (%) debe ser > 0',
...
CREATE
OR REPLACE FUNCTION "public"."abrir_nuevo_credito"(
  "p_cliente_id" bigint,
  "p_monto_solicitado" numeric,
  "p_plazo_dias" integer,
  "p_fecha_primer_pago" "date"
) RETURNS "void" LANGUAGE "plpgsql" AS $ $ BEGIN -- Archiva el crédito anterior
...
ALTER FUNCTION "public"."abrir_nuevo_credito"(
  "p_cliente_id" bigint,
  "p_monto_solicitado" numeric,
  "p_plazo_dias" integer,
  "p_fecha_primer_pago" "date"
...
CREATE
OR REPLACE FUNCTION "public"."actualizar_estado_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ declare cid bigint := coalesce(new.cliente_id, old.cliente_id);
...
select
  coalesce(max(numero_cuota), 0) into ultima_pagada
from
  cronograma
where
  cliente_id = cid
...
if todas_pagadas then estado_final := 'completo';

elsif cuotas_vencidas > 0 then estado_final := 'atrasado';

elsif cuota_hoy_pend then estado_final := 'pendiente';

else estado_final := 'al_dia';
...
CREATE
OR REPLACE FUNCTION "public"."actualizar_saldo_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ begin
update
  clientes
set
...
CREATE
OR REPLACE FUNCTION "public"."crear_cronograma_para_cliente"("p_cliente_id" bigint) RETURNS "void" LANGUAGE "plpgsql" AS $ $ declare v_plazo integer;
...
CREATE
OR REPLACE FUNCTION "public"."crear_historial_cerrado"("p_cliente_id" bigint) RETURNS "void" LANGUAGE "plpgsql" AS $ $ DECLARE v_cli RECORD;

BEGIN -- Cargamos solo lo que necesitamos de la tabla clientes
SELECT
  monto_solicitado,
  fecha_primer_pago,
  plazo_dias INTO v_cli
FROM
  public.clientes
WHERE
  id = p_cliente_id;
...
CREATE
OR REPLACE FUNCTION "public"."crear_historial_cerrado"(
  "p_cliente_id" bigint,
  "p_monto_orig" numeric,
  "p_fecha_inicio" "date",
  "p_plazo_dias" integer
) RETURNS "void" LANGUAGE "plpgsql" AS $ $ DECLARE v_total_pagado numeric;
...
BEGIN -- total pagado hasta ahora
SELECT
  COALESCE(SUM(monto_pagado), 0) INTO v_total_pagado
FROM
  public.pagos
WHERE
  cliente_id = p_cliente_id;
...
SELECT
  COALESCE(
    MAX(GREATEST((p.fecha_pago :: date - cr.fecha_venc), 0)),
    0
  ) INTO v_max_atraso
FROM
  public.pagos p
...
-- incidencias
SELECT
  COUNT(*) INTO v_count_incid
FROM
  public.historial_eventos
WHERE
  cliente_id = p_cliente_id
...
ALTER FUNCTION "public"."crear_historial_cerrado"(
  "p_cliente_id" bigint,
  "p_monto_orig" numeric,
  "p_fecha_inicio" "date",
  "p_plazo_dias" integer
...
CREATE
OR REPLACE FUNCTION "public"."crear_historial_cerrado_v1"("p_cliente_id" bigint) RETURNS "void" LANGUAGE "plpgsql" AS $ $ DECLARE v_cli RECORD;
...
CREATE
OR REPLACE FUNCTION "public"."recalcular_datos_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ declare cliente_id_var bigint := coalesce(new.cliente_id, old.cliente_id);
...
begin -- a) Suma de pagos
select
  coalesce(sum(monto_pagado), 0) into total_pagado
from
  pagos
where
  cliente_id = cliente_id_var;

-- b) Última cuota
select
  coalesce(max(numero_cuota), 0) into ultima_cuota_pagada
from
  pagos
where
  cliente_id = cliente_id_var;
...
select
  date(fecha_primer_pago),
  total_pagar into fecha_inicio,
  total_a_pagar
from
  clientes
where
...
select
  max(fecha_pago) into fecha_ult_pago_ts
from
  pagos
where
  cliente_id = cliente_id_var;
...
dias_transcurridos := date(timezone('America/Lima', now())) - fecha_inicio;

-- f) Lógica de estado:
if total_pagado >= total_a_pagar then estado := 'completo';

atraso := 0;

-- si el último pago fue justo AYER (cuota = dias_transcurridos)
elsif ultima_cuota_pagada = dias_transcurridos
and date(timezone('America/Lima', fecha_ult_pago_ts)) = date(timezone('America/Lima', now())) - 1 then estado := 'pendiente';

atraso := 0;

-- si falta alguna cuota anterior a HOY
elsif ultima_cuota_pagada < dias_transcurridos then estado := 'atrasado';

atraso := dias_transcurridos - ultima_cuota_pagada;

else -- ya pagó la cuota de hoy o se adelantó
estado := 'al_dia';
...
CREATE
OR REPLACE FUNCTION "public"."regenerar_cronograma_on_update"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ declare v_changed boolean := false;

begin if OLD.monto_solicitado <> NEW.monto_solicitado then v_changed := true;

end if;

if OLD.plazo_dias <> NEW.plazo_dias then v_changed := true;

end if;

if OLD.fecha_primer_pago <> NEW.fecha_primer_pago then v_changed := true;
...
CREATE
OR REPLACE FUNCTION "public"."validar_y_recalcular_cliente"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ declare v_tasa numeric;
...
begin if NEW.monto_solicitado <= 0 then raise exception 'El monto_solicitado (%) debe ser > 0',
NEW.monto_solicitado;

end if;

if NEW.plazo_dias not in (12, 24) then raise exception 'El plazo_dias (%) no es válido (solo 12 o 24)',
NEW.plazo_dias;

end if;

if NEW.fecha_primer_pago < now() :: date + 1 then raise exception 'La fecha_primer_pago (%) debe ser ≥ mañana',
NEW.fecha_primer_pago;
...
NEW.fecha_final := NEW.fecha_primer_pago :: date + (NEW.plazo_dias - 1) * interval '1 day';
...
CREATE
OR REPLACE FUNCTION "public"."validar_y_recalcular_cliente_upd"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ begin return public.validar_y_recalcular_cliente();
...
ALTER TABLE
  "public"."cliente_historial" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."cliente_historial_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
  "public"."cliente_historial_id_seq" OWNER TO "postgres";
...
ALTER TABLE
  "public"."clientes" OWNER TO "postgres";
...
ALTER TABLE
  "public"."clientes_backup" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."clientes_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
  "public"."clientes_id_seq" OWNER TO "postgres";
...
ALTER TABLE
  "public"."cronograma" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."cronograma_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
  "public"."cronograma_id_seq" OWNER TO "postgres";
...
ALTER TABLE
  "public"."historial_eventos" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."historial_eventos_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
  "public"."historial_eventos_id_seq" OWNER TO "postgres";
...
ALTER TABLE
  "public"."pagos" OWNER TO "postgres";

CREATE SEQUENCE IF NOT EXISTS "public"."pagos_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1;

ALTER TABLE
  "public"."pagos_id_seq" OWNER TO "postgres";

ALTER SEQUENCE "public"."pagos_id_seq" OWNED BY "public"."pagos"."id";

CREATE
OR REPLACE VIEW "public"."v_cliente_historial_completo" AS WITH "pagos_detalle" AS (
  SELECT
    "p"."cliente_id",
    "min"("cr"."fecha_venc") AS "fecha_inicio",
    "max"(
      (
        "p"."fecha_pago" AT TIME ZONE 'America/Lima' :: "text"
      )
    ) AS "fecha_cierre_real",
  ...
  FROM
    (
      "public"."pagos" "p"
      JOIN "public"."cronograma" "cr" ON (
        (
          ("p"."cliente_id" = "cr"."cliente_id")
          AND ("p"."numero_cuota" = "cr"."numero_cuota")
        )
      )
    )
...
ALTER TABLE
  "public"."v_cliente_historial_completo" OWNER TO "postgres";

CREATE
OR REPLACE VIEW "public"."v_clientes_con_estado" AS WITH "mora" AS (
  SELECT
    "cr"."cliente_id",
    "count"(*) FILTER (
      WHERE
        (
          (
            "cr"."fecha_venc" < (("now"() AT TIME ZONE 'America/Lima' :: "text")) :: "date"
          )
    ...
    (
      EXISTS (
        SELECT
          1
        FROM
          "public"."cronograma" "c2"
        WHERE
          (
            ("c2"."cliente_id" = "cr"."cliente_id")
            AND (
              "c2"."fecha_venc" = (("now"() AT TIME ZONE 'America/Lima' :: "text")) :: "date"
            )
      ...
      NOT (
        EXISTS (
          SELECT
            1
          FROM
            "public"."cronograma" "c3"
          WHERE
            (
              ("c3"."cliente_id" = "cr"."cliente_id")
...
"ranked_scores" AS (
  SELECT
    ...
    "row_number"() OVER (
      ...
      ORDER BY
        "ch"."fecha_cierre" DESC
    ) AS "rn"
  FROM
    "public"."cliente_historial" "ch"
),
"agg_scores" AS (
  SELECT
    "rs"."cliente_id",
...
SELECT
  ...
  CASE
    WHEN (
      "cli"."fecha_primer_pago" > (("now"() AT TIME ZONE 'America/Lima' :: "text")) :: "date"
    ) THEN 'proximo' :: "text"
...
ALTER TABLE
  "public"."v_clientes_con_estado" OWNER TO "postgres";

CREATE
OR REPLACE VIEW "public"."v_creditos_cerrados" AS
SELECT
  "cliente_historial"."id" AS "historial_id",
...
FROM
  "public"."cliente_historial"
ORDER BY
  "cliente_historial"."fecha_cierre";

ALTER TABLE
  "public"."v_creditos_cerrados" OWNER TO "postgres";

ALTER TABLE
  ONLY "public"."cliente_historial"
ALTER COLUMN
  "id"
SET
  DEFAULT "nextval"(
    '"public"."cliente_historial_id_seq"' :: "regclass"
  );

ALTER TABLE
  ONLY "public"."clientes"
ALTER COLUMN
  "id"
SET
...
ALTER TABLE
  ONLY "public"."cronograma"
ALTER COLUMN
  "id"
SET
  DEFAULT "nextval"('"public"."cronograma_id_seq"' :: "regclass");

ALTER TABLE
  ONLY "public"."historial_eventos"
ALTER COLUMN
  "id"
SET
  DEFAULT "nextval"(
    '"public"."historial_eventos_id_seq"' :: "regclass"
  );
...
ALTER COLUMN
  "id"
SET
  DEFAULT "nextval"('"public"."pagos_id_seq"' :: "regclass");
...
CREATE
OR REPLACE TRIGGER "trg_clientes_ai"
AFTER
INSERT
  ON "public"."clientes" FOR EACH ROW EXECUTE FUNCTION "public"."_trigger_generar_cronograma"();

CREATE
OR REPLACE TRIGGER "trg_clientes_bi_insert" BEFORE
INSERT
  ON "public"."clientes" FOR EACH ROW EXECUTE FUNCTION "public"."_validar_y_recalcular_cliente"();

CREATE
OR REPLACE TRIGGER "trg_clientes_bi_update" BEFORE
UPDATE
  ON "public"."clientes" FOR EACH ROW
  WHEN (
    (
      (
        "old"."fecha_primer_pago" IS DISTINCT
        FROM
          "new"."fecha_primer_pago"
      )
      OR (
        "old"."monto_solicitado" IS DISTINCT
        FROM
          "new"."monto_solicitado"
      )
      OR (
        "old"."plazo_dias" IS DISTINCT
        FROM
          "new"."plazo_dias"
      )
    )
  ) EXECUTE FUNCTION "public"."_validar_y_recalcular_cliente"();

CREATE
OR REPLACE TRIGGER "trg_clientes_term_update" BEFORE
UPDATE
  ON "public"."clientes" FOR EACH ROW
  WHEN (
    (
...
CREATE
OR REPLACE TRIGGER "trg_log_pago"
AFTER
INSERT
  ON "public"."pagos" FOR EACH ROW EXECUTE FUNCTION "public"."_trigger_log_pago"();

CREATE
OR REPLACE TRIGGER "trg_pagos_aiud"
AFTER
INSERT
  OR DELETE ON "public"."pagos" FOR EACH ROW EXECUTE FUNCTION "public"."_trigger_actualizar_estado"();

CREATE
OR REPLACE TRIGGER "trg_pagos_cierre"
AFTER
INSERT
  ON "public"."pagos" FOR EACH ROW EXECUTE FUNCTION "public"."_trigger_cierre_historial"();

CREATE
OR REPLACE TRIGGER "trg_refinanciar_cliente"
AFTER
UPDATE
  OF "monto_solicitado",
  "plazo_dias",
  "fecha_primer_pago" ON "public"."clientes" FOR EACH ROW
  WHEN (
    (
      (
        "old"."monto_solicitado" IS DISTINCT
...
ALTER TABLE
  ONLY "public"."cliente_historial"
ADD
  CONSTRAINT "cliente_historial_cliente_id_fkey" FOREIGN KEY ("cliente_id") REFERENCES "public"."clientes"("id") ON DELETE CASCADE;

ALTER TABLE
  ONLY "public"."cronograma"
ADD
  CONSTRAINT "cronograma_cliente_id_fkey" FOREIGN KEY ("cliente_id") REFERENCES "public"."clientes"("id") ON DELETE CASCADE;

ALTER TABLE
  ONLY "public"."historial_eventos"
ADD
  CONSTRAINT "historial_eventos_cliente_id_fkey" FOREIGN KEY ("cliente_id") REFERENCES "public"."clientes"("id") ON DELETE CASCADE;

ALTER TABLE
  ONLY "public"."pagos"
ADD
  CONSTRAINT "pagos_cliente_id_fkey" FOREIGN KEY ("cliente_id") REFERENCES "public"."clientes"("id") ON DELETE CASCADE;
...
GRANT ALL ON FUNCTION "public"."abrir_nuevo_credito"(
  "p_cliente_id" bigint,
  "p_monto_solicitado" numeric,
  "p_plazo_dias" integer,
  "p_fecha_primer_pago" "date"
...
GRANT ALL ON FUNCTION "public"."crear_historial_cerrado"(
  "p_cliente_id" bigint,
  "p_monto_orig" numeric,
  "p_fecha_inicio" "date",
  "p_plazo_dias" integer
