...
DROP TABLE IF EXISTS public.cronograma CASaCADE;
...
-- ==========================================================
CREATE
OR REPLACE FUNCTION public._validar_y_recalcular_cliente() RETURNS trigger LANGUAGE plpgsql AS $ $ DECLARE v_tasa NUMERIC;
...
IF v_fp_peru_date < v_today THEN RAISE EXCEPTION 'fecha_primer_pago (%) debe ser ≥ hoy (Lima: %)',
...
NEW.fecha_final := (
  NEW.fecha_primer_pago AT TIME ZONE 'America/Lima'
) :: date + (NEW.plazo_dias - 1) * INTERVAL '1 day';
...
CREATE TRIGGER trg_clientes_bi BEFORE
INSERT
  ON public.clientes FOR EACH ROW EXECUTE PROCEDURE public._validar_y_recalcular_cliente();

-- ==========================================================
-- 4. BEFORE UPDATE: validar y recalcular si cambian términos
-- ==========================================================
CREATE
OR REPLACE FUNCTION public._validar_y_recalcular_cliente_upd() RETURNS trigger LANGUAGE plpgsql AS $ $ BEGIN IF NEW.monto_solicitado <> OLD.monto_solicitado
OR NEW.plazo_dias <> OLD.plazo_dias
OR NEW.fecha_primer_pago <> OLD.fecha_primer_pago THEN RETURN public._validar_y_recalcular_cliente();
...
CREATE TRIGGER trg_clientes_bu BEFORE
UPDATE
  ON public.clientes FOR EACH ROW EXECUTE PROCEDURE public._validar_y_recalcular_cliente_upd();
...
IF v_fp > v_hoy THEN -- Aún no toca la primera cuota
v_estado := 'proximo';
...
AFTER
INSERT
  ON public.clientes FOR EACH ROW EXECUTE PROCEDURE public._trigger_generar_cronograma();

-- ==========================================================
-- 6. AFTER UPDATE: regenerar cronograma si cambian términos
-- ==========================================================
CREATE
OR REPLACE FUNCTION public._trigger_regenerar_cronograma() RETURNS trigger LANGUAGE plpgsql AS $ $ BEGIN IF OLD.monto_solicitado <> NEW.monto_solicitado
...
AFTER
UPDATE
  ON public.clientes FOR EACH ROW EXECUTE PROCEDURE public._trigger_regenerar_cronograma();
...
BEGIN -- 1) Sincronizar fecha_pagado en cronograma
IF TG_OP = 'INSERT' THEN
UPDATE
  public.cronograma
SET
  fecha_pagado = (NEW.fecha_pago AT TIME ZONE 'America/Lima') :: date
WHERE
  cliente_id = cid
...
SELECT
  (fecha_primer_pago AT TIME ZONE 'America/Lima') :: date INTO v_fp
FROM
  public.clientes
WHERE
  id = cid;
...
IF v_fp > v_hoy THEN est_final := 'proximo';
...
AFTER
INSERT
  OR DELETE ON public.pagos FOR EACH ROW EXECUTE PROCEDURE public._trigger_actualizar_estado();

-- ==========================================================
-- 8. (Opcional) Deshabilitar RLS mientras pruebas
-- ==========================================================
ALTER TABLE
  public.clientes DISABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.cronograma DISABLE ROW LEVEL SECURITY;

ALTER TABLE
  public.pagos DISABLE ROW LEVEL SECURITY;
...
-- 3) Máximo días de atraso
SELECT
  MAX(dias_atraso) INTO v_max_atraso
FROM
  public.clientes
WHERE
  id = p_cliente_id;

-- 4) Conteo de incidencias
SELECT
  COUNT(*) INTO v_count_incidencias
FROM
...
-- 5) Calcula score (ejemplo: -2 pts/día atraso, -5 pts/incidencia)
v_score := GREATEST(
  0,
  LEAST(
    100,
...
-- 6) Inserta el registro de cierre
INSERT INTO
...
VALUES
  (
    ...
    v_count_incidencias,
    NULL,
    -- o concatena aquí todas las observaciones si quieres
    v_score
  );
...
BEGIN -- Sólo dispara cuando número de cuota = plazo_dias del cliente
SELECT
...
AFTER
INSERT
  ON public.pagos FOR EACH ROW EXECUTE PROCEDURE public._trigger_cierre_historial();